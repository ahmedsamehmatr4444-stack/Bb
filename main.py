...
def save_fingerprint():
    ...
    # البحث عن تطابق مع مطرودين سابقين
    ...
    # البحث عن تطابق مع مستخدمين آخرين (غير مطرودين)
    c.execute('''SELECT user_id, username FROM users WHERE (device_uuid=? OR canvas_hash=?) AND user_id!=? AND status!='rejected' ''', 
              (device_uuid, data['canvas_hash'], user_id))
    normal_match = c.fetchone()
    
    # جلب رقم الهاتف وتقييم الوهمية
    ...
    # بناء ملاحظة الأمان بناءً على المطابقات
    if banned_match:
...
def handle_contact(message):
    ...
    # فحص الأرقام الوهمية (قائمة موسعة)
    ...
    # استخدام WebApp بدلاً من رابط عادي
    markup = InlineKeyboardMarkup()
...
if __name__ == '__main__':
    # للتشغيل المحلي (اختياري) – نستخدم polling
    print("Running locally with polling...")
    bot.remove_webhook()
    bot.infinity_polling(skip_pending=True)
else:
    # عند التشغيل عبر gunicorn (على Railway)، نضبط webhook أولاً
    set_webhook()    conn.close()
