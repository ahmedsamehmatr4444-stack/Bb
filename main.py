...
if __name__ == '__main__':
    # إذا تم تشغيل الملف مباشرة (python app.py)
    # نبدأ البوت في خيط منفصل، ثم نشغل Flask على المنفذ المحدد
    bot_thread = threading.Thread(target=start_bot, daemon=True)
    bot_thread.start()
    port = int(os.environ.get('PORT', 5000))
    app.run(host='0.0.0.0', port=port)
else:
    # عندما يتم استيراد الملف بواسطة gunicorn، نبدأ البوت في خيط منفصل أيضاً
    # نستخدم daemon=False لأن gunicorn قد ينهي العملية إذا كان كل الخيوط daemon
    bot_thread = threading.Thread(target=start_bot, daemon=False)
    bot_thread.start()<html lang="ar" dir="rtl">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>توثيق الاتحاد العربي | Arab Union</title>
